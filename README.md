...
Player Object: My Player.cs script implements the Player's Position attribute as _playerTransform.position and Speed as _speed. The Movement [WASD] action uses Input.GetAxisRaw("Horizontal/Vertical") in Update(), calling MovePlayer() for position updates. Plant seeds [Space] is handled by PlantSeed().
